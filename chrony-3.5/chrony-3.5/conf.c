...
//zcj added for control T1 random
static int t1_random =0;
...
CNF_ParseLine(const char *filename, int number, char *line)
{
  ...
  //zcj added for contronl T1 random 
  if (!strcasecmp(command, "t1_random")) 
  {
  	parse_int(p,&t1_random);
  }
  else if (!strcasecmp(command, "acquisitionport")) {
